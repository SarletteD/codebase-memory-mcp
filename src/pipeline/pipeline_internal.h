...
typedef enum {
    /* no declared receiver type, it does not resolve, or a base in its
     * hierarchy does not (or the walk hit its cap) and the method was not
     * found: generic path */
    CBM_ST_CALL_UNTYPED = 0,
    CBM_ST_CALL_FOUND, /* *out_method is the exact Method */
    /* the whole hierarchy resolved and none of it has the method: no edge */
    CBM_ST_CALL_NOT_FOUND,
